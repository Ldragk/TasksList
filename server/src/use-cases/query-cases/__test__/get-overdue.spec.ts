...
describe("get by overdue", () => {
  ...
  it("should return all overdue tasks", async () => {
    ...
    const { tasks } = await overdueTasks.execute(task.userId);

    expect(tasksRepository.tasks[0].date.value).toEqual(OverdueTask.date.value);
    expect(overdueTasks.execute(task.userId)).toEqual(Promise.resolve([OverdueTask]));
  ...
  it("should return all overdue tasks", async () => {
    ...
    expect(tasksQueryRepositoryMock.findByOverdue).toHaveBeenCalledWith(task.userId, false)
